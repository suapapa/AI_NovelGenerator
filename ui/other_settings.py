...
def build_other_settings_tab(self):
    self.other_settings_tab = self.track_tab(self.tabview, "other", "tab.other")
    ...
    def run_webdav_task(worker, success_message, success_callback=None):
        ...
        def finish(success, result):
            ...
            if success:
                ...
                messagebox.showinfo(t("title.success"), success_message)
            else:
                messagebox.showerror(t("title.error"), t("webdav.unknown_error", error=result))
    ...
    def test_webdav_connection():
        ...
        run_webdav_task(worker, t("webdav.test_ok"), lambda _result: save_webdav_settings())
    ...
    def backup_to_webdav():
        ...
        run_webdav_task(worker, t("webdav.backup_ok"))
    ...
    def restore_from_webdav():
        ...
        run_webdav_task(worker, t("webdav.restore_ok"), on_success)
    ...
    dav_title = ctk.CTkLabel(dav_frame, text=t("webdav.title"), font=("Microsoft YaHei", 16, "bold"))
    dav_title.pack(anchor="w", padx=5, pady=(0, 5))
    self.bind_i18n(dav_title, "webdav.title")
    ...
    create_label_with_help(self, parent=dav_warp_frame, label_key="webdav.url", tooltip_key="webdav_url",row=0, column=0, font=("Microsoft YaHei", 12), sticky="w")
    ...
    create_label_with_help(self, parent=dav_warp_frame, label_key="webdav.username", tooltip_key="webdav_username",row=1, column=0, font=("Microsoft YaHei", 12), sticky="w")
    ...
    create_label_with_help(self, parent=dav_warp_frame, label_key="webdav.password", tooltip_key="webdav_password",row=2, column=0, font=("Microsoft YaHei", 12), sticky="w")
    ...
    # 测试连接按钮
    test_btn = ctk.CTkButton(button_frame, text=t("webdav.test"), font=("Microsoft YaHei", 12),
                            command=test_webdav_connection)
    test_btn.pack(side="left", padx=5)
    self.bind_i18n(test_btn, "webdav.test")
    
    # 保存设置按钮
    save_btn = ctk.CTkButton(button_frame, text=t("webdav.backup"), font=("Microsoft YaHei", 12),
                            command=backup_to_webdav)
    save_btn.pack(side="left", padx=5)
    self.bind_i18n(save_btn, "webdav.backup")
    
    # 重置按钮
    reset_btn = ctk.CTkButton(button_frame, text=t("webdav.restore"), font=("Microsoft YaHei", 12),
                             command=restore_from_webdav)
    reset_btn.pack(side="left", padx=5)
    self.bind_i18n(reset_btn, "webdav.restore")
